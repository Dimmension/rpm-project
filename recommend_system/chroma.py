...
class ChromaDBManager:
    ...
    def add_to_collection(self, user_data: dict):
        ...
        try:
            ...
            user = self.collection.get(ids=[str(user_data["user_id"])], include=['metadatas'])
            print(user['metadatas'])
    ...
    def get_recommends_by_id(self, user_data: dict):
        ...
        try:
            ...
            print(f'RECCOMMENDS {recommended_ids[0]}')
    ...
    def add_user_like(self, user_data: dict):
        user = self.collection.get(ids=[str(user_data["user_id"])], include=['metadatas'])
        user['metadatas'][0].update({str(user_data["target_user_id"]): ' '})
        self.collection.update(
            ids=[str(user_data['user_id'])],
            metadatas=user['metadatas']
        )
        print(f"LIKE ADDED SUCCESSFULLY NOW: {user['metadatas']}")

    def check_user_likes(self, user_data: dict) -> bool:
        u_id = str(user_data['user_id'])
        t_id = str(user_data['target_user_id'])
        
        user = self.collection.get(ids=[str(user_data["user_id"])], include=['metadatas'])
        target_user = self.collection.get(ids=[str(user_data["target_user_id"])], include=['metadatas'])

        print(f'IDS: {u_id} {t_id}')
        if t_id in user['metadatas'][0].keys() and u_id in target_user['metadatas'][0].keys():
            del user['metadatas'][0][t_id]
            del target_user['metadatas'][0][u_id]
            ...
            self.collection.update(
                ids=[t_id],
                metadatas=target_user['metadatas']
            )
            print(f"RETURNING TRUE FOR PAIR {u_id} AND {t_id}")
            return True
        print(f"RETURNING FALSE FOR PAIR {u_id} AND {t_id}")
        return False
