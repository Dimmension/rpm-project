...
def add_fake_users(redis_manager, num_users=10):
    ...
    for _ in range(num_users):
        ...
        try:
            ...
            logging.info(f"Fake user added: {user_data['username']}")
        except Exception as error:
            logging.error(f"Failed to add fake user: {error}")
...
async def handle_get_recommends(message: RecMessage):
    ...
    if message['action'] == 'get_recommendations':
        ...
        for recommendation in recommendations:
            ...
            logging.info(f"Recommendations for user {message['user_id']} published successfully.")
