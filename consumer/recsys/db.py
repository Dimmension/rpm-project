...
class RedisDBManager:
    ...
    def initialize_client(self):
        ...
        try:
            ...
            logging.info(f"Redis client initialized on {self.redis_host}:{self.redis_port}.")
        except Exception as error:
            logging.error(f"Failed to initialize Redis client: {error}")
            raise error
    ...
    def add_to_collection(self, user_data: Dict):
        ...
        try:
            ...
            logging.info(f"Data for user {user_data['user_id']} added to Redis.")
        except Exception as error:
            logging.error(f"Failed to add data to Redis: {error}")
            raise error
    ...
    def add_user_like(self, user_data: dict):
        ...
        try:
            ...
            logging.info(f"User {user_data['user_id']} liked user {user_data['target_user_id']}.")
        except Exception as error:
            logging.error(f"Failed to add like: {error}")
            raise error
    ...
    def check_user_likes(self, user_data: dict) -> bool:
        ...
        try:
            ...
            if liker_likes and target_likes:
                ...
                logging.info(f"Match found between user {user_data['user_id']} and user {user_data['target_user_id']}.")
                return True

            logging.info(f"No match found between user {user_data['user_id']} and user {user_data['target_user_id']}.")
            return False

        except Exception as error:
            logging.error(f"Error in check_user_likes: {error}")
            return False
    ...
    def get_recommends_by_id(self, user_id: int) -> List[Dict]:
        ...
        try:
            ...
            if not user_data:
                logging.warning(f"No metadata found for user {user_id}.")
                return []
            ...
            logging.info(f"Potential recommendations for user {user_id}: {similar_users}")
            return similar_users

        except Exception as error:
            logging.error(f"Error in get_recommends_by_id: {error}")
            return []
